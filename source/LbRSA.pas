...
type
  PMultipDataForThread = ^MultipDataForThread;

  MultipDataForThread = record
    bi: TLbBigInt;
    RandomInterarions: byte;
  end;
...
type
  ...
  public {methods}
    ...
    function OutBufSizeNeeded(InBufSize: cardinal): cardinal;
      override;
    procedure RSACallback(var Abort: boolean);
    procedure CopyFrom(aSourceKey: TLbRSA);
    function EqualityTo(aKey: TLbRSA): boolean;
...
type
  ...
  public {methods}
    ...
    function VerifyBuffer(const Buf; BufLen: cardinal): boolean;
      override;
...
begin
  ...
  if (biBlock.GetByteValue(i) <> byte(bt01)) and
    (biBlock.GetByteValue(i) <> byte(bt02)) then
    raise Exception.Create(sRSADecodingErrIBT);
  Dec(i);

  { count padding bytes }
  while (biBlock.GetByteValue(i) <> 0) do
  begin
    Dec(i);
    if (i <= 0) then
...
{ -------------------------------------------------------------------------- }
procedure RSAFormatBlock(biBlock: TLbBigInt;
  BlockType: TRSABlockType);
begin
  ...
  { append padding }
  while (biBlock.Int.IntBuf.dwLen - biBlock.Int.dwUsed) > 2 do
  begin {!!.02}
...
{ -------------------------------------------------------------------------- }
procedure RSAEncryptBigInt(biBlock: TLbBigInt;
  Key: TLbRSAKey; BlockType: TRSABlockType; Encrypt: boolean);
var
  dwSize, dwLen: DWORD;
  tmp1, tmp2: TLbBigInt;
begin
...
begin
  ...
  try
    ...
    if (p.Compare(q) = 0) then
      raise Exception.Create('Random number are equal');
...
function EncryptRSAEx(PublicKey: TLbRSAKey;
  pInBlock, pOutBlock: PByteArray; InDataSize: integer): longint;
...
begin
  Result := EncryptRSAEx(PublicKey, @InBlock, @OutBlock,
    SizeOf(InBlock));
end;

{ -------------------------------------------------------------------------- }
function DecryptRSA(PrivateKey: TLbRSAKey;
  const InBlock: TRSACipherBlock;
...
begin
  InStream := TFileStream.Create(InFile, fmOpenRead or
    fmShareDenyWrite);
  try
...
{ == TLbRSA ================================================================ }
constructor TLbRSA.Create(AOwner: TComponent; aName: string;
  aKeySize: TLbAsymKeySize);
  { initialize }
begin
...
constructor TLbRSA.LoadFromStream(aStream: TStream);
var
  vKeySize: TLbAsymKeySize;
  vName: string;
begin
  vName := aStream.ReadAnsiString;
  vKeySize := TLbAsymKeySize(aStream.ReadByte);
  self.Create(nil, vName, vKeySize);
  self.PublicKey.ModulusAsString := aStream.ReadAnsiString;
  self.PublicKey.ExponentAsString := aStream.ReadAnsiString;
  self.FPrivateKey.ModulusAsString := aStream.ReadAnsiString;
  self.FPrivateKey.ExponentAsString := aStream.ReadAnsiString;
end;

procedure TLbRSA.PackToStream(aStream: TStream);
begin
  aStream.WriteAnsiString(FName);
  aStream.WriteByte(Ord(FKeySize));
  aStream.WriteAnsiString(self.PublicKey.ModulusAsString);
  aStream.WriteAnsiString(self.PublicKey.ExponentAsString);
  aStream.WriteAnsiString(self.FPrivateKey.ModulusAsString);
  aStream.WriteAnsiString(self.FPrivateKey.ExponentAsString);
end;
...
procedure TLbRSA.CopyFrom(aSourceKey: TLbRSA);
begin
  self.FName := aSourceKey.FName;
  self.KeySize := aSourceKey.KeySize;
  self.PublicKey.ExponentAsString :=
    aSourceKey.PublicKey.ExponentAsString;
  self.PublicKey.ModulusAsString :=
    aSourceKey.PublicKey.ModulusAsString;
  self.PrivateKey.ExponentAsString :=
    aSourceKey.PrivateKey.ExponentAsString;
  self.PrivateKey.ModulusAsString :=
    aSourceKey.PrivateKey.ModulusAsString;
end;

function TLbRSA.EqualityTo(aKey: TLbRSA): boolean;
begin
  if not (self.KeySize = aKey.KeySize) then exit(False);
  if not (self.FName = aKey.FName) then exit(False);
  if not (self.PublicKey.ExponentAsString =
    aKey.PublicKey.ExponentAsString) then
    exit(False);
  if not (self.PublicKey.ModulusAsString =
    aKey.PublicKey.ModulusAsString) then
    exit(False);
  if not (self.PrivateKey.ExponentAsString =
    aKey.PrivateKey.ExponentAsString) then
    exit(False);
  if not (self.PrivateKey.ModulusAsString =
    aKey.PrivateKey.ModulusAsString) then
    exit(False);
...
{ -------------------------------------------------------------------------- }
procedure TLbRSASSA.EncryptHash(const HashDigest;
  DigestLen: cardinal);
...
{ -------------------------------------------------------------------------- }
function TLbRSASSA.VerifyBuffer(const Buf;
  BufLen: cardinal): boolean;
...
begin
  case FHashMethod of
    ...
    begin
      ...
      Result := CompareBuffers(MD5Digest1, MD5Digest2,
        SizeOf(TMD5Digest));
    end;
    hmSHA1:
    begin
      ...
      Result := CompareBuffers(SHA1Digest1, SHA1Digest2,
        SizeOf(TSHA1Digest));
    end;
    else
      Result := False;
