...
type
  ...
  public
    ...
    function ModInv(aModulus: TLbBigInt): boolean;
    procedure PowerAndMod(Exponent: TLbBigInt; aModulus: TLbBigInt);
...
begin
  ...
  Result := N1.IntBuf.pBuf[pred(place)];
end;
...
begin
  ...
  while (N1.IntBuf.pBuf[pred(N1.dwUsed)] = 0) do
  begin
    Dec(N1.dwUsed);
...
begin
  ...
  begin
    ...
    N1.IntBuf.pBuf[N1.dwUsed] := _byte;
    Inc(N1.dwUsed);
  end
  else
  begin
    ...
    N1.IntBuf.pBuf[pred(place)] := _byte;
    if (N1.dwUsed < place) then
      N1.dwUsed := place;
  end;
end;
...
begin
  ...
  cnt := N1.dwUsed;
  while N1.IntBuf.pBuf[pred(cnt)] = N2.IntBuf.pBuf[pred(cnt)] do
  begin
    Dec(cnt);
    if (cnt = 0) then
    begin
  ...
  if N1.IntBuf.pBuf[pred(cnt)] > N2.IntBuf.pBuf[pred(cnt)] then
    Result := cGREATER_THAN
  else
    Result := cLESS_THAN;
end;
...
begin
  LbBiTrimSigZeros(N1);
  Result := False;
  if (N1.dwUsed = 1) and (N1.IntBuf.pBuf[0] = 0) then
    Result := True;
end;
...
begin
  LbBiTrimSigZeros(N1);
  Result := False;
  if (N1.dwUsed = 1) and (N1.IntBuf.pBuf[0] = 1) then
    Result := True;
end;
...
begin
  ...
  byteShr := _shr div 8;
  bitShr := _shr mod 8;
  if (byteShr > integer(N1.dwUsed)) then
  begin
    LbBiClear(N1);
  ...
  carry := 0;
  try
    for i := N1.dwUsed downto 1 do
    begin
      if (i - byteShr) < 1 then break;
      tmp := N1.IntBuf.pBuf[pred(i)];
...
begin
  ...
  begin
    ...
    Carry := tmp_wrd shr 8;
    Sum.IntBuf.pBuf[Sum.dwUsed] := tmp_byt;
    Inc(Sum.dwUsed);
  end;
...
begin
  ...
  for cnt := 0 to x do
  begin
    tmp := N1.IntBuf.pBuf[cnt];
    if (N2.dwUsed < succ(cnt)) then
      tmp := tmp - Borrow
    else
      tmp := tmp - (N2.IntBuf.pBuf[cnt] + Borrow);
    ...
    Diff.IntBuf.pBuf[Diff.dwUsed] := tmp;
    Inc(Diff.dwUsed);
  end;
  if (Borrow <> 0) then
...
begin
  ...
  MaxX := pred(N1.dwUsed);
  tmp := pred(N2.dwUsed);
  for InxY := 0 to tmp do
  begin
    if N2.IntBuf.pBuf[InxY] <> 0 then
    begin
      Carry := 0;
      for InxX := 0 to MaxX do
      begin
        plc := InxX + InxY;
        prd := N1.IntBuf.pBuf[InxX];
        prd := prd * N2.IntBuf.pBuf[InxY];
        if (Product.dwUsed < plc) then
          prd := prd + Carry
        else
          prd := prd + Product.IntBuf.pBuf[plc] + Carry;
...
begin
  ...
  //if (MilliSecondsBetween(Now, startTime) > 1)  then Writeln('n.Multiply(q) ',
...
begin
  ...
  for cnt := 0 to tmp do
  begin
    prd := (N1.IntBuf.pBuf[cnt] * N2) + Carry;
    byt := prd and $00FF;
    Carry := prd shr 8;
    Product.IntBuf.pBuf[cnt] := byt;
...
{ ------------------------------------------------------------------- }
procedure LbBiDivByDigitBase(N1: LbInteger; N2: byte; var quotient: LbInteger;
  var remainder: byte);
var
  factor: byte;
  c: integer;
...
begin
  ...
  try
    ...
    if lclDVD.IntBuf.pBuf[pred(lclDVD.dwUsed)] >= divisor then
    begin
    ...
    for c := tmp downto 0 do
    begin
      sigDivd := (Carry shl 8) or (integer(lclDVD.IntBuf.pBuf[c])); {!!03}
      ...
      if (lclQT <> 0) then
      begin
        quotient.IntBuf.pBuf[plc] := lclQT;
...
{ ------------------------------------------------------------------- }
procedure LbBiDivByDigit(N1: LbInteger; N2: byte; var quotient: LbInteger;
  var remainder: byte);
begin
...
{ ------------------------------------------------------------------- }
procedure LbBiDivBase(N1: LbInteger; N2: LbInteger; var quotient: LbInteger;
...
begin
  ...
  try
    ...
    tmpByte := lclDSR.IntBuf.pBuf[pred(lclDSR.dwUsed)];
    if (tmpByte = 0) then
    ...
    *************************************************************************}
    if lclDVD.IntBuf.pBuf[pred(lclDVD.dwUsed)] >=
      lclDSR.IntBuf.pBuf[pred(lclDSR.dwUsed)] then
    begin
      LbBiAddByte(lclDVD, cAPPEND_ARRAY, $00);
    end;
    ...
    sigDigit := lclDSR.IntBuf.pBuf[pred(lclDSR.dwUsed)];
    if (sigDigit = 0) then
    begin
      tmpInt := pred(lclDSR.dwUsed);
      while sigDigit = 0 do
      begin
        sigDigit := lclDSR.IntBuf.pBuf[tmpInt];
        Dec(tmpInt);
        if tmpInt < 0 then
          raise Exception.Create(sBIQuotientErr);
      end;
    ...
    begin
      if (lclDVD.dwUsed = 1) then
        sigDivd := lclDVD.IntBuf.pBuf[0]
      else
        sigDivd := integer(lclDVD.IntBuf.pBuf[InxX])        {!!03} shl
          8 + lclDVD.IntBuf.pBuf[pred(InxX)];
      ...
      begin
        ...
        while (LbBiCompare(lclDVD, tmpDR) = cLESS_THAN) do
        begin
          Dec(lclQT);
    ...
    begin
      ...
      begin
        tmpByte := remainder.IntBuf.pBuf[0];
...
{ ------------------------------------------------------------------- }
procedure LbBiDiv(N1: LbInteger; N2: LbInteger; var quotient: LbInteger;
  var remainder: LbInteger);
begin
...
{ ------------------------------------------------------------------- }
procedure LbBiPowerAndMod(I1: LbInteger; exponent: LbInteger;
  modulus: LbInteger; var _Result: LbInteger);
var
  BitCount: integer;
  i: integer;
  tmp_byte: byte;
...
{ ------------------------------------------------------------------- }
procedure LbBiPowerAndModInPLace(var I1: LbInteger; exponent: LbInteger;
  modulus: LbInteger);
var
  _Result: LbInteger;
begin
...
begin
  ...
  try
    for cnt := 0 to cTotalSimple2KPrimes do
    begin
      LbBiClear(N2);
      ...
      if (not LbBiIsZero(quotient)) and (not LbBiIsOne(quotient)) and
        LbBiIsZero(remBN) then
      begin
        Result := False;
        exit;
      end;
...
begin
  ...
  try
    ...
    a := 0;
    while (not LbBiGetBit(wMinus1, a)) do
    begin
      LbBiShr(m, 1);
    ...
    begin
      ...
      begin
        ...
        if ((j = 0) and LbBiIsOne(z)) or (LbBiCompare(z, wMinus1) =
          cEQUAL_TO) then
        begin
          if i < n then
          begin { inc i and start over }
            Inc(i);
            break;
          end
          else
          begin { probably prime }
            Result := False;
            exit;
          end;
        end
...
var
  ...
  len: integer;
  passed: boolean;
  CheckSimplePrimesResult, IsCompositFastResult: boolean;
  //CheckSimplePrimesTime, IsCompositFastTime, startTime: TDateTime;
begin
  ...
  try
    ...
    begin
      ...
      //CheckSimplePrimesTime := 0;
      //IsCompositFastTime := 0;
      repeat

        //startTime := Now;
        CheckSimplePrimesResult := LbBiCheckSimplePrimes(N1);
        //CheckSimplePrimesTime += Now - startTime;

        if CheckSimplePrimesResult then
        begin

          //startTime := Now;
          IsCompositFastResult := LbBiIsCompositFast(N1, Iterations);
          //IsCompositFastTime += Now - startTime;

          if (not IsCompositFastResult) then
          begin
            passed := True;
            break;
          end;
  ...
  //Writeln('CheckSimplePrimesTime ',
  //  FormatDateTime('nn:ss:zzzz', CheckSimplePrimesTime), ' ms');
  //Writeln('IsCompositFastTime ',
  //  FormatDateTime('nn:ss:zzzz', IsCompositFastTime), ' ms');
end;
...
begin
  ...
  RandomBytes(N1.IntBuf.pBuf^, Count);
  N1.dwUsed := Count;
...
procedure LbBiExtendedEuclid(var u: LbInteger; var v: LbInteger;
  var u1: LbInteger; var u2: LbInteger; var GCD: LbInteger);
var
...
begin
  ...
  try
    ...
    k := 0;
    while ((LbBiIsEven(u)) and (LbBiIsEven(v))) do
    begin
      LbBiShr(u, 1);
      LbBiShr(v, 1);
      k := 1;
    ...
    if (LbBiCompare(u, v) = cLESS_THAN) then
    begin
      LbBiSwap(u, v);
    end;
    ...
    repeat { while t3 > 0 }
      repeat { while LbBiIsEven(GCD) }
        if (LbBiIsEven(GCD)) then
        begin
          if (LbBiIsOdd(u1) or LbBiIsOdd(u2)) then
          begin
            LbBiAddInPlace(u1, v);
            LbBiAddInPlace(u2, u);
          end;
        ...
        if (LbBiIsEven(t3) or (LbBiCompare(GCD, t3) = cLESS_THAN)) then
        begin
          LbBiSwap(u1, t1);
          LbBiSwap(u2, t2);
          LbBiSwap(GCD, t3);
      ...
      while ((LbBiCompare(u1, t1) = cLESS_THAN) or
          (LbBiCompare(u2, t2) = cLESS_THAN)) do
      begin
        LbBiAddInPlace(u1, v);
        LbBiAddInPlace(u2, u);
      end;
...
{ ------------------------------------------------------------------- }
function TLbBigInt.ModInv(aModulus: TLbBigInt): boolean;
var
  tmp: LbInteger;
  x: LbInteger;
begin
  ...
  try
    LbBiCopy(tmp, aModulus.Int, aModulus.Int.dwUsed);
    LbBiVerify(tmp);
...
{ ------------------------------------------------------------------- }
procedure TLbBigInt.PowerAndMod(Exponent: TLbBigInt; aModulus: TLbBigInt);
begin
  LbBiPowerAndModInPLace(FI, Exponent.Int, aModulus.Int);
end;
